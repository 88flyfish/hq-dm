<template>
  <li v-if="data.children.length > 0" class="tree-node">
    <ul @click="selected(data.id,$event)" @mouseover="mouseover"
      ...
      <template v-for="field in fields">
        ...
        <li v-else-if="field.field == 'operationField'" style="box-sizing: border-box;height:41px;"
  ...
  <li v-else @click="selected(data.id,$event)" @mouseover="mouseover"
      @mouseout="mouseout" :sn="data.sn" class="normal-node">
    <ul>
      <template v-for="field in fields">
        <li v-if="field.isCheckbox" :style="field.fieldStyle">
          <input type="checkbox"/>
